...
func searchCti(_ *cobra.Command, args []string) error {
	...
	if fetchMeta.OutDated() {
		return fmt.Errorf("Failed to search command. err: SchemaVersion is old. SchemaVersion: %+v", map[string]uint{"latest": models.LatestSchemaVersion, "DB": fetchMeta.SchemaVersion})
	}
	...
	case "cti":
		if len(args[1:]) == 1 {
			...
			if err != nil {
				return fmt.Errorf("Failed to search CTI. err: %w", err)
			}
			...
			if err != nil {
				return fmt.Errorf("Failed to search CTIs. err: %w", err)
			}
	...
	case "cve":
		if len(args[1:]) == 1 {
			...
			if err != nil {
				return fmt.Errorf("Failed to get CTI. err: %w", err)
			}
			if len(techniques) == 0 {
				return nil
			}
	...
	case "attacker":
		...
		if err != nil {
			return fmt.Errorf("Failed to get attackers. err: %w", err)
		}
		if len(attackers) == 0 {
			return nil
		}
