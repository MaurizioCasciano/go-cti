...
func fetchMitreCti(_ *cobra.Command, _ []string) (err error) {
	if err := utils.SetLogger(viper.GetBool("log-to-file"), viper.GetString("log-dir"), viper.GetBool("debug"), viper.GetBool("log-json")); err != nil {
		return fmt.Errorf("Failed to SetLogger. err: %w", err)
	}

	driver, locked, err := db.NewDB(viper.GetString("dbtype"), viper.GetString("dbpath"), viper.GetBool("debug-sql"), db.Option{})
	if err != nil {
		if locked {
			return fmt.Errorf("Failed to initialize DB. Close DB connection before fetching. err: %w", err)
		}
	...
	if err != nil {
		return fmt.Errorf("Failed to get FetchMeta from DB. err: %w", err)
	}
	if fetchMeta.OutDated() {
		return fmt.Errorf("Failed to Insert CVEs into DB. err: SchemaVersion is old. SchemaVersion: %+v", map[string]uint{"latest": models.LatestSchemaVersion, "DB": fetchMeta.SchemaVersion})
	}
	// If the fetch fails the first time (without SchemaVersion), the DB needs to be cleaned every time, so insert SchemaVersion.
	if err := driver.UpsertFetchMeta(fetchMeta); err != nil {
		return fmt.Errorf("Failed to upsert FetchMeta to DB. dbpath: %s, err: %w", viper.GetString("dbpath"), err)
	}
	...
	if err != nil {
		return fmt.Errorf("Failed to fetch Cyber Threat Intelligence. err: %w", err)
	}
	...
	if err := driver.InsertCti(techniques, mappings, attackers); err != nil {
		return fmt.Errorf("Failed to insert. dbpath: %s, err: %w", viper.GetString("dbpath"), err)
	}
